...
public class DataSourceConfiguration {
    ...
    private DataSource buildDataSource() throws SQLException {
        ...
        dataSourceMap.put("test1", createDataSource("test1"));   //只使用一个数据库
        ...
        // 具体分库分表策略，按什么规则来分
        /**
         * 增加两个分库分表条件，分库根据id,分表根据age
         */
        ShardingRule shardingRule = ShardingRule.builder()
                .dataSourceRule(dataSourceRule)
                .tableRules(Arrays.asList(orderTableRule))
                .databaseShardingStrategy(new DatabaseShardingStrategy("id", new ModuleDatabaseShardingAlgorithm()))
                .tableShardingStrategy(new TableShardingStrategy("age", new ModuleTableShardingAlgorithm())).build();
    ...
    private static DataSource createDataSource(final String dataSourceName) {
        ...
        result.setUrl(String.format("jdbc:mysql://localhost:3306/%s", dataSourceName + "?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC"));
        result.setUsername("root");
        result.setPassword("redhat");
        return result;
    }
